...
def multi_positive_cross_entropy_loss(logits, pos_mask):
    ...
    - Unlike the previous implementation, the log is applied inside the sum over positives.
    """
    logits_max, _ = torch.max(logits, dim=1, keepdim=True)
    logits = logits - logits_max.detach()  # Numerical stability
    exp_logits = torch.exp(logits)
    ...
    # Sum over all pairs (denominator of softmax)
    all_exp_sum = exp_logits.sum(dim=1, keepdim=True)  # Keep dim for broadcasting

    # Compute per-pair log probability
    log_prob = logits - torch.log((all_exp_sum + 1e-12))  # Log inside sum

    # Compute loss: sum over positive pairs first, then average
    loss_per_sample = -(pos_mask * log_prob).sum(dim=1) / pos_mask.sum(dim=1).clamp(min=1)
...
#     # Sum over all pairs (denominator of softmax)
#     all_exp_sum = (exp_logits*torch.logical_not(pos_mask)).sum(dim=1, keepdim=True)  # Keep dim for broadcasting

    
#     logits_mask = torch.eye(len(pos_mask[0,:]), device=pos_mask.device, dtype=torch.bool)
#     logits = logits[logits_mask]
#     # Compute per-pair log probability
#     log_prob = logits - torch.log((all_exp_sum + 1e-12))  # Log inside sum

#     # Compute loss: sum over positive pairs first, then average
#     loss_per_sample = -(log_prob).sum(dim=1)
