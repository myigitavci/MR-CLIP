...
def add_dropout(model, p=0.4):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.MultiheadAttention):
            if not hasattr(module, 'dropout_layer'):
                module.dropout_layer = nn.Dropout(p)
                print(f"Added dropout to {name}")

        if isinstance(module, nn.Sequential):  # Checking MLP
            if "c_proj" in dict(module.named_children()):
                if not hasattr(module, 'dropout_layer'):
                    module.add_module("dropout", nn.Dropout(p))
                    print(f"Added dropout to {name}")
...
def main(args):
    ...
    if args.textdropout is not None:
        add_dropout(model.transformer,args.textdropout)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.visiononly:
            train_one_epoch_vision_only(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=writer)
        else:
        ...
        if args.save_logs:
            ...
            if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
                if args.visiononly:
                    evaluate_vision_only(model, data, completed_epoch, args, tb_writer=writer, tokenizer=tokenizer)
                else:
